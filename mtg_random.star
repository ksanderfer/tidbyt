            ...
            children = [
                ...
                render.Padding(
                    ...
                    child = render.Marquee(
                        ...
                        child = render.Column(
                            children = render_details(card, show_rarity) + [render_line_break()] + render_details(card, show_rarity),
                        ),
                    ),
                ),
            ],
...
def render_creature_properties(card):
    ...
    if card["power"] or card["toughness"]:
        creature_properties = render.WrappedText(
            font = APP_FONT,
            content = card["power"] + "/" + card["toughness"] + "v 1.2",
        )

    return creature_properties

# Set render for card oracle text
def render_oracle_text(card):
    oracle_text = None

    if card["text"]:
        oracle_text = render.WrappedText(
            font = APP_FONT,
            content = card["text"],
        )
...
# Render card details
def render_details(card, show_rarity):
    return [
        render.WrappedText(
            font = APP_FONT,
            content = card["type"],
